function tpos=gridspringfit_ND2(mesh,rad,spos3,dpos3,params)
%
% 
...
if ~isfield(params,'anchorpts'),anchorpts=[];else anchorpts=params.anchorpts;end
if ~isfield(params,'anchor'),anchor=[];else anchor=params.anchor;end
...
if isfield(params,'k')
    if isfield(params.k,'h')
        k.h=params.k.h;
    end
    if isfield(params.k,'nn')
        k.nn=params.k.nn;
    end
    if isfield(params.k,'a')
        k.a=params.k.a;
    end
end
...
grayLevel=0.5;
set(gcf,'Color',[1 1 1]);
h=patch('Faces',elemb,'Vertices',nodesb,'EdgeColor','none',...
