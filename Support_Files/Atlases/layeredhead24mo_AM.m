...
[T1,infoT1] = LoadVolumetricData(['nihpd_24mo_t1w'], [],'nii');
[T2,infoT2] = LoadVolumetricData(['nihpd_24mo_t2w'], [],'nii');
[T3,infoT3] = LoadVolumetricData(['nihpd_24mo_pdw'], [],'nii');
[T4,infoT4] = LoadVolumetricData(['nihpd_24mo_mask'], [],'nii');
T1 = T1./max(T1(:));
T2 = T2./max(T2(:));
T3 = T3./max(T3(:));
T4 = T4./max(T4(:));
...
%%
brainfilter =  +(brain3>0.8);
PlotSlices(brainfilter,infoT1,p)

%%
WM0 = +((brain2<0.65 & brain2>0.55))-CSF1;
...
affine_2127 = affine_nihpd_atlas;
mask = affine3d_img(mask,infoT1,infoT1,affine_2127,'neareast');

save('24mo_4layer_Mask.mat','mask','infoT1')