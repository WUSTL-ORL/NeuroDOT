...
% Put mesh back in coordinate space
...
%the array has yet to be lined up correctly, which you'll do in the next section with AlignMe
...
% Run makeAnirfast to get Green's functions only
Ti=tic;[~,dim,~,Gs,Gd,dc]=makeAnirfaster(mesh,flags); % Generate Green's functions for every measurement pair
disp(['<makeAnirfast took ',num2str(toc(Ti))])


%% Choose subset of measurements to include in A
% Set subset of measurements based off of Source-Detector Separation
keep = info.pairs.r3d <= 40; %40mm separation 

% Modify info.pairs to reflect subset of measurements
temp = info.pairs;
temp.r3d=[temp.r3d(keep)];
temp.r2d = temp.r3d;
temp.NN = [temp.NN(keep)];
temp.Src = [temp.Src(keep)];
temp.Det = [temp.Det(keep)];
temp.WL = [temp.WL(keep)]; % Get correct size
temp.lambda = [temp.lambda(keep)]; % Get correct size
temp.Mod = [temp.Mod(keep,:)];

% Re-populate info structure with pruned measurements
info.pairs = temp;
flags.infoA=info; % Place updated info in flags structure


%% Generate A
[A,Gsd]=g2a_200225(Gs,Gd,dc,dim,flags); % size(A)= [Nwl*Nmeas, Nvox]
...
Gsd_wl = [Gsd(info.pairs.WL == 1), Gsd(info.pairs.WL == 2)];
figure;semilogy(info.pairs.r3d(1:Nmeas),Gsd_wl,'*');
