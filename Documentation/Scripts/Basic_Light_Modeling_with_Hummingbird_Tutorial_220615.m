...
%% Load a Segmented Volume and generate low density head mesh
...
if ~exist('mesh','var')
    [mask,infoT1]=LoadVolumetricData(['Segmented_MNI152nl_on_MNI111'],[],'4dfp');
    meshLD=load_mesh(meshname);
    meshLD.nodes=change_space_coords(meshLD.nodes,infoT1,'coord');
    PlotMeshSurface(meshLD,pM)
end
...
PlotMeshSurface(visMeshHD,pM) %Visualize in index space
...
PlotMeshSurface(visMeshHD,pM) %visualize in coordinate space
...
% rename info file specifically for this light model
infoHD = info;

% histogram of number of measurements per SD vs radius between SD pairs
figure;histogram(infoHD.pairs.r3d,1000);xlabel('R_S_D');ylabel('N_m_e_a_s');
...
keep=infoHD.pairs.NN<=5;
temp=table;
temp.Src=infoHD.pairs.Src(keep);
temp.Det=infoHD.pairs.Det(keep);
temp.NN=infoHD.pairs.NN(keep);
temp.WL=infoHD.pairs.WL(keep);
temp.lambda=infoHD.pairs.lambda(keep);
temp.Mod=infoHD.pairs.Mod(keep);
temp.r2d=infoHD.pairs.r2d(keep);
temp.r3d=infoHD.pairs.r3d(keep);
infoHD.pairs=temp;
A_1t5NN=A(keep,:); %A with only 1st to 5th NN

save(['A_',flags.tag,'.mat'],'A','infoHD','-v7.3') %save A
save(['A_first_to_fifth_NN_',flags.tag,'.mat'],'infoHD','-v7.3') %save A with only 1st to 5th NN


%% Visualize aspects of sensitivity profile
t1=affine3d_img(mask,infoT1,dim,eye(4)); % put anatomical volume in dim space

keep=infoHD.pairs.WL==2 & infoHD.pairs.Src==1 & infoHD.pairs.Det==6; % SD pair set here
...
% FFR
keep=(infoHD.pairs.WL==2 & infoHD.pairs.r2d<=40);
a=squeeze(A(keep,:));
